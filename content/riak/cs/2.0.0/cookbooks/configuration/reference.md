...
enable new features for the current version to better support <a href="{{<baseurl>}}riak/cs/2.0.0/cookbooks/rolling-upgrades">rolling upgrades</a>. New installs shouldn't need to modify this. If you're performing a rolling upgrade, keep the original value (if not defined, Riak CS uses <code>0</code>) of the old <code>app.config</code> until all nodes have been upgraded. At that point, set to the new value
</td>
<td>
<code></code>
</td>
