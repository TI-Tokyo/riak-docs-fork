...
aliases:
  ...
  - /riak/kv/3.2.3/ops/running/nodes/renaming
---
...
Previous to Riak version 1.2, a cluster node's name could only be changed with the [`riak admin reip`]({{<baseurl>}}riak/kv/3.2.3/using/admin/riak-admin/#reip) command, which involves downtime for the entire cluster. As of Riak version 1.2, that method has been superseded by [`riak admin cluster force-replace`]({{<baseurl>}}riak/kv/3.2.3/using/admin/riak-admin/#cluster-force-replace), which is safer and does not require cluster wide downtime.
