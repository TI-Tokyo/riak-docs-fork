...
aliases:
  ...
  - /riak/kv/3.2.3/ops/running/handoff/
---
...
Hinted handoff occurs when a [vnode]({{<baseurl>}}riak/kv/3.2.3/learn/glossary/#vnode) temporarily takes over responsibility for some data and then returns that data to its original "owner." Imagine a 3-node cluster with nodes A, B, and C. If node C goes offline, e.g. during a network partition, nodes A and B will pick
...
It occurs when a [vnode]({{<baseurl>}}riak/kv/3.2.3/learn/glossary/#vnode) no longer belongs to the node on which it's running. This typically happens when the very
...
[configuration files]({{<baseurl>}}riak/kv/3.2.3/configuring/reference/#intra-cluster-handoff)
...
duration that a [vnode]({{<baseurl>}}riak/kv/3.2.3/learn/glossary/#vnode) can be blocked by multiplying the
`handoff.max_rejects` setting by the value of
[`vnode_management_timer`]({{<baseurl>}}riak/kv/3.2.3/configuring/reference/#vnode_management_timer).
