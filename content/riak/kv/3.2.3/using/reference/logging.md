...
aliases:
  ...
  - /riak/kv/3.2.3/ops/running/logging
---
...
* `file` - Console logs will be emitted to a file. This is Riak's
    ...
    `./log/console.log` on an installation from [source]({{<baseurl>}}riak/kv/3.2.3/setup/installing/source), but will differ on platform-specific installation,
...
* `console` - Console logs will be emitted to standard output, which
    can be viewed by running the [`riak attach-direct`]({{<baseurl>}}riak/kv/3.2.3/using/admin/riak-cli/#attach-direct) command
