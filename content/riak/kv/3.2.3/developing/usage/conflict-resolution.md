...
aliases:
  ...
  - /riak/kv/3.2.3/dev/using/conflict-resolution
---
...
One of Riak's [central goals](../../../learn/why-riak-kv) is high availability. It was built as a [clustered]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/clusters) system in which any [node]({{<baseurl>}}riak/kv/3.2.3/learn/glossary/#node) is capable of receiving requests without requiring that
every node participate in each request.

If you are using Riak in an [eventually consistent]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/eventual-consistency) way, conflicts between object values on different nodes is
unavoidable. Often, Riak can resolve these conflicts on its own
internally if you use causal context, i.e. [vector clocks]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/causal-context#vector-clocks) or [dotted version vectors]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/causal-context#dotted-version-vectors), when updating objects. Instructions on this can be found in the section [below](#siblings).
...
1. If your data can be modeled as one of the currently available [Riak
   Data Types]({{<baseurl>}}riak/kv/3.2.3/developing/data-types), we recommend using one of these types,
...
>
> * [Using Strong Consistency]({{<baseurl>}}riak/kv/3.2.3/developing/app-guide/strong-consistency) - A guide for developers
> * [Managing Strong Consistency]({{<baseurl>}}riak/kv/3.2.3/configuring/strong-consistency) - A guide for operators
...
While Riak _does_ have a set of [defaults]({{<baseurl>}}riak/kv/3.2.3/developing/app-guide/replication-properties#available-parameters), there are a variety of general
...
[using bucket types][usage bucket types]. The most important [bucket properties]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/buckets)
...
[`last_write_wins`]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/buckets). If `last_write_wins` is set to `false`,
...
* [Java]({{<baseurl>}}riak/kv/3.2.3/developing/usage/conflict-resolution/java)
* [Ruby]({{<baseurl>}}riak/kv/3.2.3/developing/usage/conflict-resolution/ruby)
* [Python]({{<baseurl>}}riak/kv/3.2.3/developing/usage/conflict-resolution/python)
* [C#]({{<baseurl>}}riak/kv/3.2.3/developing/usage/conflict-resolution/csharp)
* [Node.js]({{<baseurl>}}riak/kv/3.2.3/developing/usage/conflict-resolution/nodejs)

In Riak versions 2.0 and later, `allow_mult` is set to `true` by default
for any [bucket types]({{<baseurl>}}riak/kv/3.2.3/developing/usage/bucket-types) that you create. This means
that if you wish to avoid client-side sibling resolution, you have a few
options:

* Explicitly create and activate [bucket types]({{<baseurl>}}riak/kv/3.2.3/developing/usage/bucket-types)
  that set `allow_mult` to `false`
* Use Riak's [Configuration Files]({{<baseurl>}}riak/kv/3.2.3/configuring/reference) to change the [default bucket properties]({{<baseurl>}}riak/kv/3.2.3/configuring/reference#default-bucket-properties) for your
...
on what value you set for `dvv_enabled`. If set to `true`, [dotted version vectors]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/causal-context#dotted-version-vectors) will be used; if set to `false` (the default), [vector clocks]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/causal-context#vector-clocks) will be used.
...
[sibling explosion]({{<baseurl>}}riak/kv/3.2.3/using/performance/latency-reduction#siblings), which can
...
Basho's official [client libraries]({{<baseurl>}}riak/kv/3.2.3/developing/client-libraries). Examples can be found for each
client library in the [Object Updates]({{<baseurl>}}riak/kv/3.2.3/developing/usage/updating-objects) document.
...
[causal context]({{<baseurl>}}riak/kv/3.2.3/learn/concepts/causal-context). This is a less likely scenario if a client updates
...
[client libraries]({{<baseurl>}}riak/kv/3.2.3/developing/client-libraries), you can find more information about getting started with your client in [Developing with Riak KV: Getting Started]({{<baseurl>}}riak/kv/3.2.3/developing/getting-started) section.
...
the entire node. Other issues include [increased cluster latency]({{<baseurl>}}riak/kv/3.2.3/using/performance/latency-reduction) as the object is replicated and out-of-memory errors.
