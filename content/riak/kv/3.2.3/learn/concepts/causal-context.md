...
aliases:
  ...
  - /riak/kv/3.2.3/theory/concepts/context
---
...
[dev key value]: {{<baseurl>}}riak/kv/3.2.3/developing/key-value-modeling
[glossary read rep]: {{<baseurl>}}riak/kv/3.2.3/learn/glossary/#read-repair
[perf latency reduc]: {{<baseurl>}}riak/kv/3.2.3/using/performance/latency-reduction
...
[usage protocol buffers]: {{<baseurl>}}riak/kv/3.2.3/developing/api/protocol-buffers
[usage updating objects]: {{<baseurl>}}riak/kv/3.2.3/developing/usage/updating-objects
...
In the case of outcome 2, the choice between **a**, **b** and **c** is determined by settings. If you set the `allow_mult` parameter to `true` for a [bucket type]({{<baseurl>}}riak/kv/3.2.3/developing/usage/bucket-types), all non-CRDT writes to that bucket type will create siblings in the case of concurrent writes (and occasionally under other
