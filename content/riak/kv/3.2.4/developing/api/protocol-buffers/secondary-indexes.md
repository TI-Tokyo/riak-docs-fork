...
aliases:
  ...
  - /riak/kv/3.2.4/dev/references/protocol-buffers/secondary-indexes
---
...
`type` | The bucket type of the bucket that is being queried. If not set, the bucket type `default` will be used. Learn more about [using bucket types]({{<baseurl>}}riak/kv/3.2.4/developing/usage/bucket-types).
...
`results` | If `return_terms` is specified with range queries, used to return matched index values as key/value pairs in `RpbPair` messages. More on `RpbPair` messages can be found in [PBC Fetch Object]({{<baseurl>}}riak/kv/3.2.4/developing/api/protocol-buffers/fetch-object).
