...
older configuration system based on [bucket properties]({{<baseurl>}}riak/kv/3.2.4/developing/usage/bucket-types/#bucket-properties-and-operations).
...
[HTTP]({{<baseurl>}}riak/kv/3.2.4/developing/api/http/set-bucket-props) or [Protocol Buffers]({{<baseurl>}}riak/kv/3.2.4/developing/api/protocol-buffers/set-bucket-props). With this approach, you can take any given bucket and
...
* Nearly all bucket properties can be updated using bucket types, except the
  ...
  [Riak data types]({{<baseurl>}}riak/kv/3.2.4/developing/data-types), and [strong consistency]({{<baseurl>}}riak/kv/3.2.4/developing/app-guide/strong-consistency) respectively
...
way that they are configured when using [bucket properties]({{<baseurl>}}riak/kv/3.2.4/developing/usage/bucket-types/#bucket-properties-and-operations). You cannot simply take a
...
>
> If you are connecting to Riak using one of Basho's official [client libraries]({{<baseurl>}}riak/kv/3.2.4/developing/client-libraries), you can find more information about getting started with your client in our [Developing with Riak KV: Getting Started]({{<baseurl>}}riak/kv/3.2.4/developing/getting-started) section.
...
Prior to Riak 2.0, Riak created [siblings]({{<baseurl>}}riak/kv/3.2.4/learn/concepts/causal-context/#siblings) in the case of conflicting updates only when explicitly instructed to do so, i.e. when `allow_mult` is to `true`. The default `allow_mult` setting was `false`.
...
do so. New applications, however, are encouraged to retain and [resolve siblings]({{<baseurl>}}riak/kv/3.2.4/developing/usage/conflict-resolution) with the appropriate application-side business logic.
...
`user_account_bucket` with a [pre-commit hook]({{<baseurl>}}riak/kv/3.2.4/developing/usage/commit-hooks/#pre-commit-hooks) called `syntax_check` and two [post-commit
hooks]({{<baseurl>}}riak/kv/3.2.4/developing/usage/commit-hooks/#Post-Commit-Hooks) called `welcome_email` and `update_registry`. This would involve four steps:
...
because buckets act as a [separate namespace](#buckets-as-namespaces) in Riak, in addition to [buckets]({{<baseurl>}}riak/kv/3.2.4/learn/concepts/buckets) and [keys]({{<baseurl>}}riak/kv/3.2.4/learn/concepts/keys-and-objects).
