...
aliases:
  ...
  - /riak/kv/3.2.4/ops/running/backups
---
...
[aae read repair]: {{<baseurl>}}riak/kv/3.2.4/learn/concepts/active-anti-entropy/#read-repair-vs-active-anti-entropy
...
Data could exist on some nodes and not others at the exact time a backup is made. Any inconsistency will be corrected once a backup is restored, either by Riak's [active anti-entropy]({{<baseurl>}}riak/kv/3.2.4/learn/concepts/active-anti-entropy/) processes or when the object is read, via [read repair]({{<baseurl>}}riak/kv/3.2.4/learn/concepts/active-anti-entropy/#read-repair-vs-active-anti-entropy).
{{% /note %}}
...
In previous versions of Riak KV, there was a [`riak admin backup`]({{<baseurl>}}riak/kv/3.2.4/using/admin/riak-admin/#backup) command commonly used for
...
1. Mark the original instance down in the cluster using
   [`riak admin down <node>`]({{<baseurl>}}riak/kv/3.2.4/using/admin/riak-admin/#down)
2. Join the restored node to the cluster using
   [`riak admin cluster join <node>`]({{<baseurl>}}riak/kv/3.2.4/using/admin/riak-admin/#cluster-join)
3. Replace the original instance with the renamed instance with
   [`riak admin cluster force-replace <node1> <node2>`]({{<baseurl>}}riak/kv/3.2.4/using/admin/riak-admin/#cluster-force-replace)
...
{{% note %}}
For more information on the `riak admin cluster` commands, refer to our documentation on [cluster administration]({{<baseurl>}}riak/kv/3.2.4/using/admin/).
{{% /note %}}
...
Restoring a cluster from backups is documented [on its own page]({{<baseurl>}}riak/kv/3.2.4/using/repair-recovery/failure-recovery/#cluster-recovery-from-backups).
