...
toc: true
aliases:
  - /riak/3.4.0/ops/running/logging
  - /riak/kv/3.4.0/ops/running/logging
---
...
As of Riak KV 3.4.0 a few of the log messages may contain newline
...
* `file` - Console logs will be emitted to a file. This is Riak's
    ...
    `./log/console.log` on an installation from [source]({{<baseurl>}}riak/kv/3.4.0/setup/installing/source), but will differ on platform-specific installation,
...
* `console` - Console logs will be emitted to standard output, which
    can be viewed by running the [`riak attach-direct`]({{<baseurl>}}riak/kv/3.4.0/using/admin/riak-cli/#attach-direct) command
