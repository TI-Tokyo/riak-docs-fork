...
toc: true
aliases:
  - /riak/3.4.0/ops/building/basic-cluster-setup
  - /riak/kv/3.4.0/ops/building/basic-cluster-setup
---
...
Most configuration changes will be applied to the [configuration file]({{<baseurl>}}riak/kv/3.4.0/configuring/reference/) located in your `rel/riak/etc` directory (if
...
cluster again.
>
> *Rename the node using the [`riak admin cluster replace`]({{<baseurl>}}riak/kv/3.4.0/using/admin/riak-admin/#cluster-replace) command. This will not work if you have previously only started Riak with a single node.
...
you'll be using the default port (8087). If you're using the [Protocol Buffers interface]({{<baseurl>}}riak/kv/3.4.0/developing/api/protocol-buffers/) to Riak (which we recommend over the HTTP
...
[`riak admin cluster force-replace`]({{<baseurl>}}riak/kv/3.4.0/using/admin/riak-admin/#cluster-force-replace) the node.
...
can also find more detailed instructions about [adding and removing nodes]({{<baseurl>}}riak/kv/3.4.0/using/cluster-operations/adding-removing-nodes) from a cluster.
