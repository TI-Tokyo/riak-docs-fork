...
As an example, let's store an object containing information about a dog named Rufus. We'll store that object in the key `rufus` in the bucket `dogs`, which bears the `animals` [bucket type]({{<baseurl>}}riak/kv/3.4.0/using/cluster-operations/bucket-types).
...
Now, you run the same read operation as in [Reading Objects]({{<baseurl>}}riak/kv/3.4.0/developing/usage/reading-objects). If the write operation was successful, you should be able to successfully read the object. Please note that the operation will fail if you don't first create the bucket-type `animals` as per the page on [bucket types]({{<baseurl>}}riak/kv/3.4.0/using/cluster-operations/bucket-types).
...
[`default`]({{<baseurl>}}riak/kv/3.4.0/developing/usage/bucket-types) will be applied.
