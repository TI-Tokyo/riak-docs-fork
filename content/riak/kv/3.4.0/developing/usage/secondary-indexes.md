...
toc: true
aliases:
  - /riak/3.4.0/dev/using/2i
  - /riak/kv/3.4.0/dev/using/2i
---
...
recommend [Riak Search]({{<baseurl>}}riak/kv/3.4.0/developing/usage/search/) rather than secondary indexes for
...
be used to find multiple objects in Riak. If you're storing [user data]({{<baseurl>}}riak/kv/3.4.0/developing/data-modeling/#user-accounts), for example, you could tag each object
...
[Riak Search]({{<baseurl>}}riak/kv/3.4.0/developing/usage/search/) serves analogous purposes but is quite
...
* Query results can be used as input to a [MapReduce]({{<baseurl>}}riak/kv/3.4.0/developing/usage/mapreduce/)
  query
...
[strongly consistent buckets]({{<baseurl>}}riak/kv/3.4.0/developing/app-guide/strong-consistency/#creating-a-strongly-consistent-bucket-type) and attach
...
mechanism that does not require a schema (as does [Riak Search]({{<baseurl>}}riak/kv/3.4.0/using/reference/search/#schemas)) and a basic query interface, i.e. an interface that
...
* When you need more than the exact match and range searches that 2i
  supports. If that's the case, we recommend checking out [Riak Search]({{<baseurl>}}riak/kv/3.4.0/developing/usage/search/).
* When you want to use composite queries. A query like
  ...
  [MapReduce]({{<baseurl>}}riak/kv/3.4.0/developing/usage/mapreduce/)).
...
>
> If you are connecting to Riak using one of Basho's official [client libraries]({{<baseurl>}}riak/kv/3.4.0/developing/client-libraries), you can find more information about getting started with
your client in the [Developing with Riak KV: Getting Started]({{<baseurl>}}riak/kv/3.4.0/developing/getting-started) section.
...
>
> For all 2i queries, the [R]({{<baseurl>}}riak/kv/3.4.0/developing/app-guide/replication-properties#r-value-and-read-failure-tolerance) parameter is set to 1,
which means that queries that are run while [handoffs]({{<baseurl>}}riak/kv/3.4.0/learn/glossary/#hinted-handoff) and related operations are underway may not
