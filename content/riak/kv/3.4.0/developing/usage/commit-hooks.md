...
toc: true
aliases:
  - /riak/3.4.0/dev/using/commit-hooks
  - /riak/kv/3.4.0/dev/using/commit-hooks
---
...
Pre- and post-commit hooks are applied at the [bucket]({{<baseurl>}}riak/kv/3.4.0/learn/concepts/buckets) level,
...
Because hooks are defined at the bucket level, you can create [bucket types]({{<baseurl>}}riak/kv/3.4.0/developing/usage/bucket-types)
...
documentation on [object deletion]({{<baseurl>}}riak/kv/3.4.0/using/reference/object-deletion)) to determine whether a delete is
occurring.
...
- `fail` - The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API]({{<baseurl>}}riak/kv/3.4.0/developing/api/http)) along with a generic error message about why the write was blocked.
