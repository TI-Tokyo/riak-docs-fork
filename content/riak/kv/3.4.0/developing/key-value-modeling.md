...
toc: true
aliases:
  - /riak/3.4.0/dev/data-modeling/key-value/
  - /riak/kv/3.4.0/dev/data-modeling/key-value/
---

While Riak enables you to take advantage of a wide variety of features
that can be useful in application development, such as [Search]({{<baseurl>}}riak/kv/3.4.0/developing/usage/search), [secondary indexes (2i)]({{<baseurl>}}riak/kv/3.4.0/developing/usage/secondary-indexes/), and [Riak Data Types]({{<baseurl>}}riak/kv/3.4.0/developing/data-types/), Riak almost always performs best when you
...
for each of them or consulting our guide to [building applications with Riak]({{<baseurl>}}riak/kv/3.4.0/developing/app-guide/) for a better sense of which features you might need.
...
* The object's [key]({{<baseurl>}}riak/kv/3.4.0/learn/concepts/keys-and-objects#keys), which can be anything you
  want as long as it is [Unicode compliant](http://www.unicode.org/)
* The [bucket]({{<baseurl>}}riak/kv/3.4.0/learn/concepts/buckets) which houses the object and its key (bucket
  names are also Unicode compliant)
* The [bucket type]({{<baseurl>}}riak/kv/3.4.0/developing/usage/bucket-types) that determines the bucket's
  [replication]({{<baseurl>}}riak/kv/3.4.0/developing/app-guide/replication-properties) and other properties
...
look like (for the [HTTP API]({{<baseurl>}}riak/kv/3.4.0/developing/api/http)):
...
One way to determine this is to [list all keys]({{<baseurl>}}riak/kv/3.4.0/developing/api/protocol-buffers/list-keys) in the
...
A better possibility is to use [Riak sets]({{<baseurl>}}riak/kv/3.4.0/developing/data-types/#sets) to
store lists of keys in a bucket. Riak sets are a [Riak Data Type]({{<baseurl>}}riak/kv/3.4.0/developing/data-types) that enable you to store lists of binaries or strings in Riak.
...
`usernames`. The following will also assume that we've [set up a bucket type]({{<baseurl>}}riak/kv/3.4.0/developing/data-types/#setting-up-buckets-to-use-riak-data-types) called
`sets`.
...
>
> If you are connecting to Riak using one of Basho's official [client libraries]({{<baseurl>}}riak/kv/3.4.0/developing/client-libraries), you can find more information about getting started with your client in [Developing with Riak KV: Getting Started]({{<baseurl>}}riak/kv/3.4.0/developing/getting-started).
...
Riak [bucket types]({{<baseurl>}}riak/kv/3.4.0/developing/usage/bucket-types) have two essential functions:
they enable you to manage [bucket configurations]({{<baseurl>}}riak/kv/3.4.0/learn/concepts/buckets) in an
...
that simply extend Riak's [defaults]({{<baseurl>}}riak/kv/3.4.0/developing/usage/bucket-types/#bucket-types-as-namespaces) or multiple bucket types that have
