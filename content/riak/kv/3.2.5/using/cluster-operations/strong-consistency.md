...
status of a node. This data is available by running the [`riak admin status`]({{<baseurl>}}riak/kv/3.2.5/using/admin/riak-admin/#status) command. That data now
includes statistics specific to strongly consistent operations.

A full listing of these stats is available in [Inspecting a Node]({{<baseurl>}}riak/kv/3.2.5/using/cluster-operations/inspecting-node).
