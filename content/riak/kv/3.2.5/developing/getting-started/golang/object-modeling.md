...
We'll be using the bucket `Users` to store our data. We won't be [using bucket types]({{<baseurl>}}riak/kv/3.2.5/developing/usage/bucket-types) here, so we don't need to specify one.
