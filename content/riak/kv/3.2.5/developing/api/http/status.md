...
The output of `/stats` contains the output of `riak admin status` detailed in the [Inspecting a Node]({{<baseurl>}}riak/kv/3.2.5/using/cluster-operations/inspecting-node) doc, plus the below stats generated by the Riak Core application.
