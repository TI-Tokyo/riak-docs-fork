...
1. On any node in your BDP cluster, run:
    ...
    `PUBLICIPOFMASTERNODE` is the address passed to [Spark master service config][bdp configure spark master].  This will start the Spark master on that machine.
...
1. Stop the service: `sudo data-platform-admin stop-service riak@»PUBLICIPOFWORKERNODE« my-spark-group my-spark-master`
1. Kill the process: `sudo pkill -f deploy.worker.Master`
1. And then restart the service: `sudo data-platform-admin start-service riak@»PUBLICIPOFWORKERNODE« my-spark-group my-spark-master`.
    {{% /note %}}
...
1. Kill the process: `sudo pkill -f deploy.worker.Worker`
1. And then restart the service: `sudo data-platform-admin start-service riak@»PUBLICIPOFWORKERNODE« my-spark-group my-spark-worker`.
    {{% /note %}}
