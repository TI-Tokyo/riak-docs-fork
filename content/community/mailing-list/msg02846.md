...
description: ""
project: community
lastmod: 2011-04-01T10:23:50-07:00
sitemap:
  priority: 0.2
layout: mailinglistitem
mailinglist_id: "msg02846"
mailinglist_parent_id: "msg02842"
author_name: "Jeff Pollard"
project_section: "mailinglistitem"
sent_date: 2011-04-01T10:23:50-07:00
---
...
&gt; the number of nodes that fail &gt; than your n\\_val ) then run with the local
...
On Fri, Apr 1, 2011 at 10:03 AM, Eric Moritz wrote:

&gt; I concur with Mark, Rackspace has some poor performance with Riak.
&gt;
&gt; On Fri, Apr 1, 2011 at 1:00 PM, Mark Steele 
&gt; wrote:
...
&gt; 2
&gt; &gt;&gt; hour period with minimal risks of a outage or drop in performance, hence
&gt; I
&gt; &gt;&gt; am guessing that maybe EC2 is not the right choice for us. We are going
&gt; to
...
&gt; on
&gt; &gt;&gt; &gt; EC2 for any kind of reason, mostly a hardware failure of the
&gt; underlying host
&gt; &gt;&gt; &gt; of course.
&gt; &gt;&gt; &gt;
&gt; &gt;&gt; &gt; Cluster compute instances offer especially high CPU power, but what
&gt; you
&gt; &gt;&gt; &gt; really want is really fast and reliable storage I/O, persisted for
&gt; eternity
&gt; &gt;&gt; &gt; if need be. CC instances are certainly a lot better than any other
&gt; instance
&gt; &gt;&gt; &gt; in that terms of general I/O (see [2] for a comparison), but fall prey
&gt; to
...
&gt; away
&gt; &gt;&gt; &gt; data durability in case of a single disk failure. One disk fails, and
&gt; the
&gt; &gt;&gt; &gt; data on that node is gone. Depending on what kinds of seek your doing,
&gt; an
&gt; &gt;&gt; &gt; EBS setup may even turn out to be faster. See [6] and [4] for a
&gt; comparison
&gt; &gt;&gt; &gt; and some initial and extended measurements, and [7] for another
&gt; comparison.
&gt; &gt;&gt; &gt; But certainly, the cluster compute instance's ephemeral storage can
&gt; achieve
&gt; &gt;&gt; &gt; a good amount of throughput, see [5] for some pretty graphs comparing
&gt; both
...
&gt; running on
&gt; &gt;&gt; &gt; raw iron servers. Both ephemeral storage and EBS don't make the
&gt; problem of a
&gt; &gt;&gt; &gt; proper backup disappear. You could e.g. run off ephemeral storage,
&gt; relying
&gt; &gt;&gt; &gt; on both Riak's replication and a good backup e.g. to an EBS volume or
&gt; to S3.
&gt; &gt;&gt; &gt;
...
&gt; increased
&gt; &gt;&gt; &gt; redundancy through mirroring. [1] has a good summary of when it's
&gt; better to
&gt; &gt;&gt; &gt; choose RAID 5 vs. 10.
&gt; &gt;&gt; &gt;
...
&gt; again
&gt; &gt;&gt; &gt; is that one corrupted volume is enough to make a RAID 0 setup
&gt; unusable.
&gt; &gt;&gt; &gt;
...
&gt; and
&gt; &gt;&gt; &gt; network storage. But know what you're getting yourself into with this
&gt; kind
...
&gt; cluster
&gt; &gt;&gt; &gt; compute instances obviously having some of the highest bandwidth
&gt; available.
&gt; &gt;&gt; &gt;
...
&gt; located
&gt; &gt;&gt; &gt; in different physical locations. Usually they're not that far apart,
&gt; but far
&gt; &gt;&gt; &gt; enough to increase latency considerably. But as always, it depends on
&gt; your
&gt; &gt;&gt; &gt; particular use case.
&gt; &gt;&gt; &gt;
...
&gt; your
&gt; &gt;&gt; &gt; data is replicated on more than one node, it can work in both ways.
&gt; You
&gt; &gt;&gt; &gt; could use both ephemeral storage and be somewhat safe because data
&gt; will
&gt; &gt;&gt; &gt; reside on multiple nodes. The same is true for EBS volumes, as
&gt; potential
&gt; &gt;&gt; &gt; variances in I/O or even minutes of total unavailabilities (as seen on
&gt; the
&gt; &gt;&gt; &gt; recent Reddit outage) can be recovered a lot easier thanks to handoff
&gt; and
&gt; &gt;&gt; &gt; read repairs. You can increase the number of replicas (n\\_val) to
&gt; increase
&gt; &gt;&gt; &gt; your tolerance of instance failure, just make sure that n\\_val is less
&gt; than
&gt; &gt;&gt; &gt; the number of nodes in your cluster.
&gt; &gt;&gt; &gt;
&gt; &gt;&gt; &gt; Don't get me wrong, I love EC2 and EBS, being able to spin up servers
&gt; at
&gt; &gt;&gt; &gt; any time and to attache more storage to a running instance is
&gt; extremely
&gt; &gt;&gt; &gt; powerful, when you can handle the downsides. But if very low latency
&gt; is what
&gt; &gt;&gt; &gt; you're looking for, raw iron with lots of memory and SSD as storage
&gt; device
...
&gt; give a
&gt; &gt;&gt; &gt; good enough performance in a lot of cases. If you decide to go with a
&gt; RAID,
&gt; &gt;&gt; &gt; be sure to add LVM on top for simpler snapshotting, which will be
&gt; quite
...
&gt; involved
&gt; &gt;&gt; &gt; when you're going under the hood, but this should cover the most
&gt; important
&gt; &gt;&gt; &gt; bases.
&gt; &gt;&gt; &gt;
&gt; &gt;&gt; &gt; Mathias Meyer
...
&gt; &gt;&gt; &gt;&gt; the feasibility of running RIAK in a EC2 cloud, as we have a
&gt; requirement
&gt; &gt;&gt; &gt;&gt; that may require us to scale up and down quite considerably on a
&gt; month by
&gt; &gt;&gt; &gt;&gt; month basis. After some initial testing and investigation we have
&gt; come to
...
&gt; &gt;&gt; &gt;&gt;
&gt; &gt;&gt; &gt;&gt; - A instance failure results in complete loss of data on that
&gt; machine,
...
&gt; with
&gt; &gt;&gt; &gt;&gt; EBS, so we would have to use multiple availability zones although
&gt; there are
&gt; &gt;&gt; &gt;&gt; only 4 in total and it just seems as though I am over complicating
&gt; things.
&gt; &gt;&gt; &gt;&gt;
...
&gt; &gt;&gt; &gt;&gt; alternatives although the Joyent solution seems very expensive, and
&gt; what are
&gt; &gt;&gt; &gt;&gt; their experiences?
&gt; &gt;&gt; &gt;&gt;
&gt; &gt;&gt; &gt;&gt; Dave
