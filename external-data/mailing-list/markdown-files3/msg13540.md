...
description: ""
project: community
lastmod: 2014-01-30T06:54:34-08:00
sitemap:
  priority: 0.2
layout: mailinglistitem
mailinglist_id: "msg13540"
mailinglist_parent_id: "msg13538"
author_name: "Guido Medina"
project_section: "mailinglistitem"
sent_date: 2014-01-30T06:54:34-08:00
---
...
allow\_multi=true won't make any sense, but look at this scenario:


All of our buckets have allow\_multi=false except for the one bucket we 
...
LWW=true, will it affect the bucket(s) with allow\_multi=true, is it safe 
to assume that if allow\_multi=true LWW will be ignored? We only modify 
...
the default value for Erlang property last\_write\_wins is false, now, 
if 95% of the buckets/keys have no siblings (or conflict resolution), 
does that mean that for such buckets last\_write\_wins is set to true, 
I'm wondering what's the effect (if any) if allow\_multi on a bucket 
is false.


In other words; I could assume that:

 \* If allow\_multi is true then last\_write\_wins will be ignored
 'cause vclock is needed for conflict resolution?
 \* if allow\_multi is false then last\_write\_wins is true?

They’re independant settings, but allow\_mult=true + lww=true makes no 
sense (in reality, in the code, I’m pretty sure the lww=true will be 
applied.)


allow\_mult=false+lww=false means at each vnode there is a 
...
means that you get \_one\_ of the causally concurrent values, the one 
with the largest timestamp.


allow\_mult=false+lww=false means that at the coordinating vnode we 
...
I really think, for riak, 90% of the time, allow\_mult=true is your 
...
Using last\_write\_wins = true, do I need to always send the vclock 
...
On 29 January 2014 10:29, Edgar Veiga > wrote:
 ...
 second interval, is last\_write\_wins=true profitable?
 ...
 On 27 January 2014 23:25, Edgar Veiga
 > wrote:

 Hi there everyone!

 I would like to know, if my current application is a
 good use case to set last\_write\_wins to true.
 ...
 last\_write\_wins to true? I've been facing some massive
