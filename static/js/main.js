  ...
  generateVersionLists = function() {
    ...
    if (project && project !== "community" && project !== "404") {
      return $.getJSON(project_descriptions_path, function(data) {
        var $version_pane, $version_pane__shadow, $version_pane__sizing, anchor, anchor_tag, archive_below, archived_class, archived_url, arrow_str, class_list, has_archived, i, in_version_range, index, is_lts, j, k, l, latest_rel, len, len1, len2, len3, list_depth, lts_name, lts_series, project_data, project_path, ref, ref1, ref2, relative_path, release_sem_ver, release_set, release_version, set_index, tallest_list, url, version_selector_list_html;
        project_data = data[project];
        project_path = docs_root_url.replace(/\/+$/, '') + project_data.path;
        latest_rel = project_data.latest;
        lts_series = project_data['lts'];
        archive_below = project_data['archive_below'];
        ...
        for (set_index = i = 0, len = ref.length; i < len; set_index = ++i) {
          ...
          is_lts = false;
          if (lts_series) {
            for (j = 0, len1 = lts_series.length; j < len1; j++) {
              lts_name = lts_series[j];
          ...
          for (index = k = 0, len2 = ref1.length; k < len2; index = ++k) {
            ...
            if (in_version_range && (release_version !== current_version)) {
              ...
              for (l = 0, len3 = versions_locations.length; l < len3; l++) {
                ref2 = versions_locations[l], range = ref2[0], url = ref2[1];
        ...
        if (has_archived) {
          class_list = ["selector-list__element", "selector-list__element--other"];
        ...
        $('.edge-fader__arrow').on('click.selector-fader-arrow', EdgeFader.onClickCallback);
        return $('.selector-other-releases').on('click', function(event) {
